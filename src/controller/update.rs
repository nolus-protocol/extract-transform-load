use std::{collections::BTreeMap, fs, io::BufReader, sync::LazyLock};
...
use crate::{
    ...
    error::Error,
    futures_set::try_join_all_with_capacity,
    model::LS_Opening,
};
...
async fn ls_loan_amnt(
    ...
    try_join_all_with_capacity(
        state
            .database
            .ls_opening
            .get_leases_data(read_leases_json().await?)
            .await?
            .into_iter()
            .map(|lease| {
...
async fn read_leases_json() -> Result<Vec<String>, Error> {
    task::spawn_blocking(move || {
        fs::File::open(concat!(env!("CARGO_MANIFEST_DIR"), "/leases.json"))
            .map_err(From::from)
            .and_then(|file| {
