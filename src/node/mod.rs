    ...
    Self: Reconnect,
{
    pub async fn connect(uri: &str) -> Result<Self> {
        const CONNECT_TO_GRPC_ERROR: &str = "Failed to connect to node's gRPC endpoint!";
...
impl ClientInner {
    ...
    async fn tx_service_client(self: &Arc<Self>) -> Result<TxServiceClient<GrpcChannel>> {
        self.reconnect_if_required().await?;
    ...
    async fn wasm_query_client(self: &Arc<Self>) -> Result<WasmQueryClient<GrpcChannel>> {
        self.reconnect_if_required().await?;
...
impl Reconnect for ClientInner {
    async fn reconnect(&self) -> Result<()> {
        const RECONNECT_ERROR: &str = "Failed to reconnect to node's gRPC endpoint!";

        let mut lock = self.grpc.write().await;

        if self.should_reconnect.load(Ordering::Acquire) {
            let new_channel = self.endpoint.connect().await.context(RECONNECT_ERROR)?;
...
fn set_reconnect_if_required(client_inner: &ClientInner, error_code: TonicCode) {
