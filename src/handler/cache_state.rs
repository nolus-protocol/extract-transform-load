...
pub async fn set_total_value_locked(app_state: AppState<State>) -> Result<(), Error> {
    let neutron_usdc_noble = if let Some((neutron_usdc_noble, _)) = app_state.config.lp_pools.get(0)
    {
        neutron_usdc_noble
    } else {
    ...
    let osmosis_usdc = if let Some((osmosis_usdc, _)) = app_state.config.lp_pools.get(1) {
        osmosis_usdc
    } else {
    ...
    let neutron_usdc_axelar =
        if let Some((neutron_usdc_axelar, _)) = app_state.config.lp_pools.get(2) {
            neutron_usdc_axelar
        } else {
            return Err(Error::ProtocolError(String::from("neutron_usdc_axelar")));
        };

    let osmosis_usdc_noble = if let Some((osmosis_usdc_noble, _)) = app_state.config.lp_pools.get(3)
    {
        osmosis_usdc_noble
    } else {
