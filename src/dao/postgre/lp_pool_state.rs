        ...
        let value: (Option<BigDecimal>,) = sqlx::query_as(
            ...
            WITH per_pool AS (
                ...
                FROM (
                    ...
                    WHERE "LP_Lender_id" = 'nolus17rjgmry3w2xcc8yer4h4m8vuypkhkh8he3u8xv'
                    AND "LP_Pool_id"   = 'nolus1qg5ega6dykkxc307y25pecuufrjkxkaggkkxh7nad0vhyhtuhw3sqaa3c5'
                    ORDER BY "LP_timestamp" DESC
                    LIMIT 1
                ) ls
                CROSS JOIN (
                    SELECT COALESCE(SUM("LP_amnt_stable"), 0)::numeric AS "deposited_raw"
                    FROM "LP_Deposit"
                    WHERE "LP_address_id" = $1
                    AND "LP_Pool_id"    = 'nolus1qg5ega6dykkxc307y25pecuufrjkxkaggkkxh7nad0vhyhtuhw3sqaa3c5'
                ) dep
                CROSS JOIN (
                    SELECT COALESCE(SUM("LP_amnt_stable"), 0)::numeric AS "withdrawn_raw"
                    FROM "LP_Withdraw"
                    WHERE "LP_address_id" = $1
                    AND "LP_Pool_id"    = 'nolus1qg5ega6dykkxc307y25pecuufrjkxkaggkkxh7nad0vhyhtuhw3sqaa3c5'
                ) wdr
                ...
                CROSS JOIN (
                    SELECT COALESCE(SUM("LP_amnt_asset"), 0)::numeric AS "deposited_asset"
                    FROM "LP_Deposit"
                    WHERE "LP_address_id" = $1
                    AND "LP_Pool_id"    = 'nolus1w2yz345pqheuk85f0rj687q6ny79vlj9sd6kxwwex696act6qgkqfz7jy3'
                ) dep
                CROSS JOIN (
                    SELECT COALESCE(SUM("LP_amnt_asset"), 0)::numeric AS "withdrawn_asset"
                    FROM "LP_Withdraw"
                    WHERE "LP_address_id" = $1
                    AND "LP_Pool_id"    = 'nolus1w2yz345pqheuk85f0rj687q6ny79vlj9sd6kxwwex696act6qgkqfz7jy3'
                ) wdr
            ...
            SELECT
                (SELECT SUM("earnings_in_stable") FROM per_pool) AS "total_earnings_in_stable"
            FROM per_pool LIMIT 1
            "#,
        )
        .persistent(false)
        .bind(address)
