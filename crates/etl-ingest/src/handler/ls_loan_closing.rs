...
use crate::provider::is_sync_running;
...
async fn proceed_repayment(
    ...
    if let Some(opened) = lease.into_opened() {
        data.insert(
            opened.amount.ticker.to_owned(),
            LS_Loan_Collect {
                LS_contract_id: ls_loan_closing.LS_contract_id.to_owned(),
                LS_symbol: opened.amount.ticker.to_owned(),
                LS_amount: BigDecimal::from_str(&opened.amount.amount)?,
                LS_amount_stable: state
                    .in_stable_by_date(
                        &opened.amount.ticker,
...
async fn proceed_market_close(
    ...
    if lease.PartialClose.is_some() {
        ...
        if let Some(opened) = lease_state.into_opened() {
            data.insert(
                opened.amount.ticker.to_owned(),
                LS_Loan_Collect {
                    LS_contract_id: ls_loan_closing.LS_contract_id.to_owned(),
...
pub async fn get_pnl_long(
    ...
    let lease_debt = BigDecimal::from_str(&lease_status.principal_due.amount)?
        + BigDecimal::from_str(&lease_status.overdue_margin.amount)?
        + BigDecimal::from_str(&lease_status.overdue_interest.amount)?
        + BigDecimal::from_str(&lease_status.due_margin.amount)?
        + BigDecimal::from_str(&lease_status.due_interest.amount)?;
