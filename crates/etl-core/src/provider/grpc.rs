...
use crate::{
    ...
    types::{
        ...
        Balance, LPP_Price, LP_Pool_Config_State_Type, LP_Pool_State_Type,
        LS_Raw_State, LS_State_Type, OracleCurrency, PlatformInfo,
        PriceAmountObject, Prices,
    },
};
...
impl Grpc {
    ...
    /// Query a lease contract for its current state.
    ///
    /// New contracts (post block 10958318) accept `{"state": {}}`.
    /// Old contracts only accept `{}`. We try the new format first
    /// and fall back to the old format on contract-side rejection.
    pub async fn get_lease_state(
        &self,
        contract: String,
    ) -> Result<LS_State_Type, Error> {
        const QUERY_CONTRACT_ERROR: &str =
            "Failed to query lease contract state!";
        const PARSE_ERROR: &str = "Failed to parse lease state response!";
        ...
        let data = match new_result {
            Ok(bytes) => bytes,
            Err(_) => {
                // Old contract version — try legacy empty query
                self.with_retry(
                    || self.wasm_query_client.clone(),
                    |mut client| {
                        let contract = contract.to_owned();
        ...
        Ok(serde_json::from_slice::<LS_State_Type>(&data)
            .context(PARSE_ERROR)?)
    }
    ...
    pub async fn get_stable_price(
        ...
        contract: String,
        ticker: String,
    ) -> Result<PriceAmountObject, Error> {
        const QUERY_CONTRACT_ERROR: &str =
        ...
        let data = self
            ...
            .context(QUERY_CONTRACT_ERROR)
            .and_then(|data| {
                serde_json::from_slice::<PriceAmountObject>(&data)
                    .context(PARCE_MESSAGE_ERROR)
            })?;
