...
/// Lease state as returned by the contract's `{"state": {}}` query.
///
/// Variants:
...
impl LS_State_Type {
    /// Returns the opened status if the lease is active, `None` otherwise.
    pub fn into_opened(self) -> Option<Status_Opened> {
        match self {
            LS_State_Type::Opened(s) => Some(*s),
            _ => None,
        }
    }
}
...
pub struct Status_Opened {
    ...
    pub overdue_margin: AmountTicker,
    pub overdue_interest: AmountTicker,
    pub due_margin: AmountTicker,
    pub due_interest: AmountTicker,
    #[serde(default)]
    pub overdue_collect_in: u128,
    #[serde(default)]
    pub due_projection_ns: u128,
    #[serde(default)]
