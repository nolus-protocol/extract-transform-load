use std::{
    collections::HashMap, env, fs, ops::Deref, str::FromStr, sync::Arc,
    time::Duration,
};
...
/// Cache TTL constants
pub const CACHE_TTL_STANDARD: Duration = Duration::from_secs(300); // 5 minutes for real-time/price-dependent data
pub const CACHE_TTL_HOURLY: Duration = Duration::from_secs(3600); // 1 hour for aggregated state data (refreshed by aggregation task)
...
pub struct ApiCache {
    ...
    pub total_value_locked: Cache<String, BigDecimal>,
    pub total_tx_value: Cache<String, BigDecimal>,
    pub realized_pnl_stats: Cache<String, BigDecimal>,
    pub revenue: Cache<String, BigDecimal>,
    pub open_position_value: Cache<String, BigDecimal>,
    pub open_interest: Cache<String, BigDecimal>,
    pub leased_assets: Cache<String, Vec<Leased_Asset>>,
    pub supplied_funds: Cache<String, BigDecimal>,
    pub positions: Cache<String, Vec<Position>>,
    pub supplied_borrowed_history: Cache<String, Vec<Supplied_Borrowed_Series>>,
    pub leases_monthly: Cache<String, Vec<Leases_Monthly>>,
    pub current_lenders: Cache<String, Vec<CurrentLender>>,
    pub liquidations: Cache<String, Vec<LiquidationData>>,
    pub lease_value_stats: Cache<String, Vec<LeaseValueStats>>,
    pub historical_lenders: Cache<String, Vec<HistoricalLender>>,
    pub loans_granted: Cache<String, Vec<LoanGranted>>,
    pub historically_liquidated: Cache<String, Vec<HistoricallyLiquidated>>,
    pub historically_repaid: Cache<String, Vec<HistoricallyRepaid>>,
    pub historically_opened: Cache<String, Vec<HistoricallyOpened>>,
    pub interest_repayments: Cache<String, Vec<InterestRepaymentData>>,
    pub realized_pnl_wallet: Cache<String, Vec<RealizedPnlWallet>>,
    // Additional cached endpoints
    pub buyback_total: Cache<String, BigDecimal>,
    pub distributed: Cache<String, BigDecimal>,
    pub incentives_pool: Cache<String, BigDecimal>,
    pub monthly_active_wallets: Cache<String, Vec<MonthlyActiveWallet>>,
    pub revenue_series: Cache<String, Vec<RevenueSeriesPoint>>,
    pub daily_positions: Cache<String, Vec<DailyPositionsPoint>>,
    pub position_buckets: Cache<String, Vec<PositionBucket>>,
    pub loans_by_token: Cache<String, Vec<TokenLoan>>,
    pub open_positions_by_token: Cache<String, Vec<TokenPosition>>,
    pub unrealized_pnl: Cache<String, BigDecimal>,
    pub pools: Cache<String, Vec<PoolUtilizationLevel>>,
    // Period-based endpoints
    pub buyback: Cache<String, Vec<Buyback>>,
    pub borrowed: Cache<String, BigDecimal>,
}
...
impl ApiCache {
    pub fn new() -> Self {
        Self {
            ...
            // =================================================================
            // From LS_State
            positions: build_cache(CACHE_TTL_HOURLY),
            open_position_value: build_cache(CACHE_TTL_HOURLY),
            open_interest: build_cache(CACHE_TTL_HOURLY),
            unrealized_pnl: build_cache(CACHE_TTL_HOURLY),
            lease_value_stats: build_cache(CACHE_TTL_HOURLY),
            position_buckets: build_cache(CACHE_TTL_HOURLY),
            open_positions_by_token: build_cache(CACHE_TTL_HOURLY),
            loans_by_token: build_cache(CACHE_TTL_HOURLY),
            daily_positions: build_cache(CACHE_TTL_HOURLY),
            // From LP_Pool_State
            supplied_borrowed_history: build_cache(CACHE_TTL_HOURLY),
            pools: build_cache(CACHE_TTL_HOURLY),
            supplied_funds: build_cache(CACHE_TTL_HOURLY),
            borrowed: build_cache(CACHE_TTL_HOURLY),
            // From LP_Lender_State
            current_lenders: build_cache(CACHE_TTL_HOURLY),

            // =================================================================
            // STANDARD TTL (5 min) - Real-time events or price-dependent data
            // =================================================================
            // Price-dependent (uses current market prices)
            total_value_locked: build_cache(CACHE_TTL_STANDARD),
            leased_assets: build_cache(CACHE_TTL_STANDARD),
            // Real-time event data
            liquidations: build_cache(CACHE_TTL_STANDARD),
            interest_repayments: build_cache(CACHE_TTL_STANDARD),
            historical_lenders: build_cache(CACHE_TTL_STANDARD),
            historically_opened: build_cache(CACHE_TTL_STANDARD),
            historically_repaid: build_cache(CACHE_TTL_STANDARD),
            historically_liquidated: build_cache(CACHE_TTL_STANDARD),
            loans_granted: build_cache(CACHE_TTL_STANDARD),
            realized_pnl_stats: build_cache(CACHE_TTL_STANDARD),
            realized_pnl_wallet: build_cache(CACHE_TTL_STANDARD),
            // Treasury (real-time events)
            buyback: build_cache(CACHE_TTL_STANDARD),
            buyback_total: build_cache(CACHE_TTL_STANDARD),
            distributed: build_cache(CACHE_TTL_STANDARD),
            incentives_pool: build_cache(CACHE_TTL_STANDARD),
            revenue: build_cache(CACHE_TTL_STANDARD),
            revenue_series: build_cache(CACHE_TTL_STANDARD),
            // Other real-time data
            total_tx_value: build_cache(CACHE_TTL_STANDARD),
            leases_monthly: build_cache(CACHE_TTL_STANDARD),
            monthly_active_wallets: build_cache(CACHE_TTL_STANDARD),
        }
    }
}
...
impl State {
    pub async fn new(
        ...
        for protocol_name in &active_protocol_names {
            ...
            let lpn_decimals = active_currencies
                .get(&lpn)
                .map(|c| c.1 as i64)
                .ok_or_else(|| Error::ConfigurationError(
                    format!("LPN currency '{}' not found in active currencies for protocol '{}'", lpn, protocol_name)
                ))?;
            ...
            let stable_currency_decimals = active_currencies
                ...
                .map(|c| c.1 as i64)
                .ok_or_else(|| Error::ConfigurationError(
                    format!("Stable currency '{}' not found in active currencies for protocol '{}'", stable_currency, protocol_name)
                ))?;
    ...
    /// Compute the fee paid at lease opening
    pub async fn get_fees(
        &self,
        lease: &crate::model::LS_Opening,
        protocol: String,
    ) -> Result<BigDecimal, Error> {
        use anyhow::Context as _;

        let symbol = &lease.LS_asset_symbol.to_owned();
        let ctrl_currency = self
            .config
            .hash_map_currencies
            .get(&lease.LS_cltr_symbol)
            .context(format!(
                "ctrl_currency not found {}",
                &lease.LS_cltr_symbol
            ))?;

        let loan_currency = self
            .config
            .hash_map_currencies
            .get(&symbol.to_owned())
        ...
        let lpn_currency =
            self.get_currency_by_pool_id(&lease.LS_loan_pool_id)?;
...
/// Since etl-core lives at `crates/etl-core/`, we go two levels up.
...
fn parse_config_vapid_keys() -> Result<(Vec<u8>, Vec<u8>), Error> {
    ...
    let private_key_dir = format!("{}cert/vapid_private.pem", root);
    let public_key_dir = format!("{}cert/vapid_public.b64", root);
...
pub fn get_configuration() -> Result<Config, Error> {
    ...
    let allowed_origins = env::var("ALLOWED_ORIGINS")?
        ...
        .collect::<Vec<String>>();
    let static_dir =
        format!("{}{}", workspace_root(), env::var("STATIC_DIRECTORY")?);
...
pub fn set_configuration() -> Result<(), Error> {
    let path = format!("{}.env", workspace_root());
    dotenvy::from_path(path).map_err(|e| {
        Error::ConfigurationError(format!("Failed to load .env: {}", e))
    })?;
    Ok(())
}
