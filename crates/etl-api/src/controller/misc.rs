...
pub async fn history_stats(
    ...
    let pnl = state
        ...
        .await
        .map_err(|e| {
            tracing::error!("history-stats: get_realized_pnl failed: {e}");
            e
        })?;
    let tx_volume = state
        .database
        .raw_message
        .get_tx_volume(address.to_owned())
        .await
        .map_err(|e| {
            tracing::error!("history-stats: get_tx_volume failed: {e}");
            e
        })?;
    let win_rate = state
        .database
        .raw_message
        .get_win_rate(address.to_owned())
        .await
        .map_err(|e| {
            tracing::error!("history-stats: get_win_rate failed: {e}");
            e
        })?;
    let bucket = state
        .database
        .raw_message
        .get_buckets(address.to_owned())
        .await
        .map_err(|e| {
            tracing::error!("history-stats: get_buckets failed: {e}");
            e
        })?;
...
#[get("/version")]
pub async fn version() -> Result<impl Responder, crate::error::ApiError> {
...
pub async fn test_push(
    ...
    push::send(state.as_ref().clone(), query.address.to_owned(), push_data)
        .await?;
